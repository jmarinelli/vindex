...
└── src/
    ...
    │   ├── dexie.ts                # Dexie database definition (IndexedDB schema + helpers)
    │   ├── sync.ts                 # Sync queue processor (findings + photos → server)
    │   ├── sync-provider.tsx       # Global <SyncProvider> context (mounted at dashboard layout)
    │   ├── photo-queue.ts          # Photo capture and compression
    │   ├── photo-upload.ts         # Cloudinary upload with retry logic
    │   └── hooks.ts                # React hooks: useDraft, useOfflineStatus, usePhotoUpload
    │
...
- `drafts` — inspection metadata (vehicle info, template snapshot, section index). No embedded findings or photos.
- `findings` — individual finding records with `syncedAt` timestamp. Unsynced = `syncedAt === null`.
- `photos` — photo records with `uploaded` flag. Blobs stored in-memory during capture, uploaded to Cloudinary.
...
- Runs on: mount, offline→online transition, and when `triggerSync()` is called.
...
- `saved` — persisted in Dexie, not yet synced.
- `syncing` — sync worker is processing the queue.
- `synced` — queue empty and online.
