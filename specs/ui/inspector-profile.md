...
│  │  años de experiencia. Inspecciones detalladas      │  │
...
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐         │  │
│  │  │    24    │  │ Mar 2024 │  │   8.3    │         │  │
│  │  │inspeccio-│  │inspeccion-│ │fotos por │         │  │
│  │  │  nes     │  │ando desde│  │ reporte  │         │  │
│  │  └──────────┘  └──────────┘  └──────────┘         │  │
│  │                                                    │  │
│  │  ┌──────────┐  ┌──────────┐                        │  │
│  │  │   5.2    │  │   4.1    │                        │  │
...
| Verified label | `text-xs`, `success` color, with shield-check icon | Below node name, same row alignment. "Inspector verificado" |
...
|------|-------|-------|-------------|
| Inspection count | Integer | "inspecciones" | Count of signed events where `node_id = this node` |
| Operating since | Month + Year | "inspeccionando desde" | `signed_at` of the earliest signed event. Format: "Mar 2024". If no inspections: hidden. |
...
| Section title | `text-base`, `font-semibold`, `gray-800` | "Inspecciones firmadas ({count})" |
...
| Text | `text-sm`, `gray-500`, text-center | "Este inspector aún no tiene inspecciones firmadas." |
...
| Title | `text-xl`, `font-bold`, `gray-800` | "Inspector no encontrado" |
...
| **Stats computation** | Inspection count is correct, "inspeccionando desde" shows earliest signed_at month/year, averages computed correctly |
...
| **Zero inspections** | Stats section hidden. Report list shows empty state message: "Este inspector aún no tiene inspecciones firmadas." |
...
- Verified label has `aria-label`: "Inspector verificado por VinDex".
...
- Stat tiles have `aria-label` combining value and label (e.g., "24 inspecciones").
